...
class CreateOriginsRequestAddresses implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['server_role']) && (mb_strlen($this->container['server_role']) < 1)) {
            $invalidProperties[] = "invalid value for 'server_role', the character length must be bigger than or equal to 1.";
        }

        if (!is_null($this->container['server_role']) && !preg_match("/^(primary|backup)$/", $this->container['server_role'])) {
            $invalidProperties[] = "invalid value for 'server_role', must be conform to the pattern /^(primary|backup)$/.";
        }
     ...
     * Sets server_role
     *
     * @param string|null $server_role server_role
     *
     * @return self
     */
    public function setServerRole($server_role)
    {
        ...
        if ((mb_strlen($server_role) > 10)) {
            throw new \InvalidArgumentException('invalid length for $server_role when calling CreateOriginsRequestAddresses., must be smaller than or equal to 10.');
        }
        if ((mb_strlen($server_role) < 1)) {
            throw new \InvalidArgumentException('invalid length for $server_role when calling CreateOriginsRequestAddresses., must be bigger than or equal to 1.');
        }
        if ((!preg_match("/^(primary|backup)$/", ObjectSerializer::toString($server_role)))) {
            throw new \InvalidArgumentException("invalid value for \$server_role when calling CreateOriginsRequestAddresses., must conform to the pattern /^(primary|backup)$/.");
        }
